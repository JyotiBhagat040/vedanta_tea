...
export default function MarkingPage() {
  ...
  const loadSale = useCallback(async (sale, batch) => {
    ...
    setFilterGrades([]); setFilterGardens([]); setFilterBrokers([]); setFilterOrigins([]); setFilterRates([]); setSearchLot('');
...
@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact;}.lbl{page-break-inside:avoid;}}
  ...
  const ratesOpts   = useMemo(() => [...new Set(allLots.map(r => fmtRateUtil(r)).filter(v => v && v !== '—'))].sort(), [allLots]);
  ...
  const filteredLots = useMemo(() => allLots.filter(row => {
    ...
    if (filterRatesSet.size   > 0 && !filterRatesSet.has(fmtRateUtil(row))) return false;   // NEW
